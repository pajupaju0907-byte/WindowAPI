...
public:
	...
	// 이 블럭의 관성모멘트(회전에 대한 저항, "회전판 질량"에 해당). 단위는 질량*픽셀^2, 물리적으로
	// 정확한(게임 느낌을 위해 축소하지 않은) 값이다.
	// [강체물리 2단계] ResolveRigidCollision이 "충돌 지점에 가해진 힘이 얼마나 회전을 만들어내는가
	// (각가속도 = 토크 / 관성모멘트)"를 계산할 때 씀. 칸별 질량은 m_cellWeight 비율을 따른다.
	// [주의] "넘어지기 쉽게" 관성을 깎는 게임 느낌 튜닝(Constants::TOPPLE_INERTIA_SCALE)은 여기 넣지
	// 말 것 — ApplyGravityTorque 호출부에서만 곱해야 한다. 여기서 깎으면 충돌 임펄스 계산에도 새어
	// 들어가서 가만히 쌓인 블럭들까지 과도하게 회전하는 버그가 생긴다(실전에서 발견되어 수정됨).
