...
namespace Constants
{
	...
	// [트리키 타워 붕괴 판정] 무게중심이 지지 범위를 벗어난 블록에 순간적으로 부여하는 각속도(도/초).
	// 예전엔 200이었는데, 절벽 가장자리처럼 넘어지자마자 옆 블록과 바로 충돌하는 상황에서 이 속도
	// 그대로 충돌 임펄스로 전달돼 옆 블록을 위로 세게 튕겨 올렸다. 자연스러운 붕괴 속도로 체감에
	// 맞춰 직접 조절할 것.
	constexpr float TUMBLE_ANGULAR_VELOCITY = 100.0f;
